...
public class ExampleRankUse {
    public static void main(String[] args) {
        ...
        api.registerCalculator("min-max", (playerData, ladderRanks, arrayData) -> {
           double min = dataList.stream().mapToDouble(PlayerData::getScore).reduce(Math::min).orElse(0.0);
           double max = dataList.stream().mapToDouble(PlayerData::getScore).reduce(Math::max).orElse(0.0);
            double v = playerData.getScore();
